...
export const ButtonCta = ({ iconLeft, variant, outline, type, children }: ButtonProps) => {
  return (
    <button
      ...
      ${variant === 'black' ? 'bg-neutral-900 text-background outline-neutral-900 hover:outline-neutral-900' : 'bg-neutral-50 text-neutral-900'} ${
        outline ? 'outline outline-1 outline-neutral-900' : '' // Adicionando a classe de borda condicional
      }`}
    >
      <div className="flex items-center justify-center gap-2">
        <div className={`absolute left-3 transform -translate-x-8 group-hover:translate-x-0 transition-transform ease-out duration-300
          ${variant === 'black' ? 'opacity-0 group-hover:opacity-100 text-background' : ''} // Inicialmente transparente e aparece no hover com text-background
        `}>
          {iconLeft}
        </div>
